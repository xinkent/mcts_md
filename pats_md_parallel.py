...
class Node:
    ...
    def MakeChild(self, s):
        n = Node(parent = self, state = s, c = self.c)
        return n
...
def parallel_MDrun(nodes):
    p = Pool(processes = N_threads)
    res = p.map(do_mdrun, nodes)
    return res
...
def UCT(rootstate):
    ...
    if cn:
        ...
        rmsd_list = []
        o = open('log_pats.txt','w')
        os.system('echo 4 4| gmx rms -s %s.gro -f %s.gro -o first_rmsd.xvg'%(target, reactant))
        first_rmsd = read_rmsd('first_rmsd.xvg')[0] 
        o.write(str(first_rmsd))
        o.close()
    for i in range(steps):
        ...
        while (node.try_num >= MAX_try) and node.childNodes != []: # node is fully expanded and non-terminal
        ...
        state = n_state + 1
        nodes = [node.MakeChild(s = state+i) for i in range(N_threads)]
        min_rmsds = parallel_MDrun(nodes)
        for node_i in range(N_threads):
            node = nodes[node_i]
            min_rmsd = min_rmsds[node_i]
